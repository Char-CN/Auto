...
import org.blazer.dataupdate.handle.bean.AUDepend;
import org.blazer.dataupdate.handle.bean.AUDependCalc;
import org.blazer.dataupdate.handle.bean.AUPrimary;
			...
			for (Map<String, Object> map : findList) {
				...
				for (String sql : selectSqls) {
					...
					for (String key : varKeyValue.keySet()) {
						sql = sql.replaceAll(key, varKeyValue.get(key));
					}
					// 查询计算sql
				...
				for (AUDepend aud : aup.getAuDepends()) {
					...
					for (String field : aud.getSelectField().split(DEPEND_SELECT_FIELD_SPLIT)) {
						...
						if (calcMap.containsKey(fields[0].trim())) {
							...
							String us = updateSql.toString().replaceAll(fields[0].trim(), replaceStr);
