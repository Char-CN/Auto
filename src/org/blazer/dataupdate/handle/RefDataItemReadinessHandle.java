...
public class RefDataItemReadinessHandle {
	...
	public void updateByMysql(final Map<String, Object> map, final StringBuffer findMaxSql) {
		...
		logger.info("UpdateByMysql RecordID=[" + recordID + "]");
		String UPDATE_SQL_BYMYSQL = null;
		try {
			...
			if ("7d".equalsIgnoreCase(StringUtil.getString(map.get("OptionAttrib")))) {
				startDate = "IFNULL(DATE_SUB((" + findMaxSql.toString() + "), INTERVAL " + ((PeriodCount * 7) - 1) + " DAY), StartPeriod)";
			}
			if ("1m".equalsIgnoreCase(StringUtil.getString(map.get("OptionAttrib")))) {
				startDate = "IFNULL(DATE_ADD(LAST_DAY(DATE_SUB((" + findMaxSql.toString() + "), INTERVAL " + (PeriodCount - 0)
						+ " MONTH)), INTERVAL 1 DAY), StartPeriod)";
			}
			if ("1y".equalsIgnoreCase(StringUtil.getString(map.get("OptionAttrib")))) {
				startDate = "IFNULL(DATE_FORMAT(DATE_SUB((" + findMaxSql.toString() + "), INTERVAL 1 YEAR), '%Y-01-01'), StartPeriod)";
			}
			if (startDate == null) {
			...
			// 更新Readiness
			UPDATE_SQL_BYMYSQL = "UPDATE RC_DataItemReadiness SET LastPeriod=(" + lastDate + "),StartPeriod=" + startDate + " WHERE 1 = 1 AND RecordID="
					+ recordID;
			UPDATE_SQL_BYMYSQL = UPDATE_SQL_BYMYSQL.replace("\r", " ").replace("\n", " ").replace("\t", " ");
			jdbcDao.update(UPDATE_SQL_BYMYSQL);
		} catch (Exception e) {
			logger.error("更新语句执行出错！OptionAttrib[" + map.get("OptionAttrib") + "]", e);
			logger.info("通过UpdateByJava再执行一次");
			updateByJava(map, findMaxSql);
		}
	}
	...
	public void updateByJava(final Map<String, Object> map, final StringBuffer findMaxSql) {
		JDBCDao jdbcDao = (JDBCDao) ApplicationUtil.getBean("JDBCDao");
		String recordID = StringUtil.getString(map.get("RecordID"));
		logger.info("UpdateByJava RecordID=[" + recordID + "]");
		try {
			Integer PeriodCount = IntegerUtil.getInteger(map.get("PeriodCount"));
			// 查询最大日期
			...
			if ("7d".equalsIgnoreCase(StringUtil.getString(map.get("OptionAttrib")))) {
				startDate = DateUtil.subDay(lastDate, 7 * number);
			}
			if ("1m".equalsIgnoreCase(StringUtil.getString(map.get("OptionAttrib")))) {
				startDate = DateUtil.subMonth(lastDate, number);
			}
			if ("1y".equalsIgnoreCase(StringUtil.getString(map.get("OptionAttrib")))) {
				startDate = DateUtil.subYear(lastDate, number);
			}
			if (startDate == null) {
				logger.info("Readiness RecordID = [" + recordID + "], startDate is null, The readiness update continue");
				return;
			...
			// 更新Readiness
			logger.info("UpdateByJava Parameter lastDate=[" + lastDate + "], startDate=[" + startDate + "], recordID=[" + recordID + "]");
			jdbcDao.update(UPDATE_SQL.toString(), lastDate, startDate, recordID);
		} catch (Exception e) {
			logger.error("UpdateByJava Exception：", e);
		}
	}
